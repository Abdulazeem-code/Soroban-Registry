...
use shared::models::{
    AuditCheckRow, CategoryScore, CheckStatus, ChecklistItem, DetectionMethod, Severity,
};
    ...
    for (category, items) in &by_category {
        ...
        for item in items {
            let s = status_map
                ...
                .map(|r| &r.status)
                .unwrap_or(&CheckStatus::Pending);
            if *s == CheckStatus::NotApplicable {
                continue;
            }
            let w = severity_weight(&item.severity);
            total_weighted_total += w;
            if *s == CheckStatus::Passed {
                total_weighted_passed += w;
            }
        }
...
pub fn build_markdown_report(
    ...
    for severity in [
        ...
        let mut section_items: Vec<(&ChecklistItem, &AuditCheckRow)> = all
            ...
            .filter_map(|item| {
                ...
                if failures_only && row.status == CheckStatus::Passed {
                    return None;
                }
                Some((item, *row))
            })
        ...
        for (item, row) in &section_items {
            ...
            if let Some(notes) = &row.notes {
                md.push_str(&format!("**Auditor Notes:** {}  \n", notes));
            }
            if let Some(evidence) = &row.evidence {
                md.push_str(&format!("**Evidence:**\n```\n{}\n```\n", evidence));
            }
            if row.auto_detected {
                md.push_str("*Auto-detected by source analysis.*  \n");
            }
            md.push('\n');
        }
    }

    let total = check_rows
        .iter()
    ...
    let passed = check_rows
        ...
        .count();
    let failed = check_rows
        .iter()
        .filter(|r| r.status == CheckStatus::Failed)
        .count();
    let pending = check_rows
        .iter()
